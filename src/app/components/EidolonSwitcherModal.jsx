...
export default function EidolonSwitcherModal({ eidolons, activeEidolonId, onSelect, onClose, onCreate, goalsCatalog = [], lockedIds = null, onPromote, atCap = false, maxEidolons = null }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 360, background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 24, maxHeight: '70vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        {onCreate && (
          <button onClick={onCreate} style={{ width: '100%', marginTop: 12, padding: '12px 16px', background: 'rgba(34,214,138,0.08)', border: '1px solid rgba(34,214,138,0.25)', borderRadius: 10, color: '#22D68A', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
